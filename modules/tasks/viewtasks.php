...
if($tasklist) {
    foreach($tasklist as $trow) {
		...
		if($trow->assigned=='0' || $trow->assigned!=$staffsess->preferred_name){
			$assignedlink = '<a href="'. $this->url('tasks', 'viewtasks', array('task' => $taskID, 'setowner' => 'takeownership')) .'">Atribuir report</a>';
		} else {
			$assignedlink = '<a href="'. $this->url('tasks', 'viewtasks', array('task' => $taskID, 'setowner' => 'release')) .'">Remover</a>';
		}
